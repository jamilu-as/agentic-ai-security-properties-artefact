...
def audit_cell(cell, config, suite, args):
    ...
    out = {k: v[0] for k, v in counts.items()}
    out.update(getattr(pipeline, "inner_counts", {}) or {})
    return out
...
AXIS_ELEMENT = {
    ...
    "spotlighting": ["ToolsExecutor", "InnerSpotlightPrompt"],
    "piguard":      ["DefenseFilterElement", "InnerDetector"],
    "camel":        ["PrivilegedLLM"],
}
...
def main():
    ...
    for cell in compose.factorial(args.model):
        cell = dataclasses.replace(cell, placement=args.placement)
