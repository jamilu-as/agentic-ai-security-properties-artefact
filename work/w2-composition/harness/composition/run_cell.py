...
def run_one(cell: Cell, suite_name: str, args) -> dict:
    ...
    # .logdir it does not have. Trajectories land under logdir; Section 3.12 withholds the raw
    # outputs from release, and .gitignore excludes them.
    from agentdojo.logging import OutputLogger
    from agentdojo import cost_tracker

    def _cost_delta(before):
        after = cost_tracker.summary()
        keys = ("total_cost_usd", "total_calls", "total_prompt_tokens", "total_completion_tokens")
        return {k.replace("total_", ""): round(after[k] - before[k], 6) for k in keys}
    cost0 = cost_tracker.summary()
    ...
    if args.utility:
        ...
        # prereg Section 7 and Section 3.9 both specify injection-free. A composed cell that has not
        ...
        return {
            ...
            "per_task_utility": {str(k): bool(v) for k, v in sorted(util.items())},
            # defender cost per benign episode, as charged by OpenRouter: the economic
            # force's numerator (prereg section 9), measured injection-free
            "cost": _cost_delta(cost0),
        }
    ...
    return {
        ...
        "composition_fingerprint": fingerprint,
        "attack": args.attack,
        "regime": "R2-adaptive" if args.attack in ("autodojo",) or args.attack.startswith("optim") else "R1-static",
        ...
        # analysis: Section 3.9 clusters on the INJECTION TASK and bootstraps by resampling
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--placement", default="pinned",
                    choices=["pinned", "inner", "detector_on_output", "prompt_on_quarantined"])
    ...
    ap.add_argument("--utility", action="store_true",
                    help="measure U_c: benign completion with NO injection, the quantity "
                         "the configuration-level utility gate needs (prereg Section 7). The "
    ...
    # defect Section 4.II.a reports in AutoDojo's own grid - metadata naming the wrong model -
