...
if __name__ == "__main__":
    ...
    # Write the counts beside the trajectories. The trajectories themselves are withheld
    # under the dual-use position of Section 3.13 - they are raw model output - so the counts are
    # the artefact Table 4.4 resolves to. Without this the table cited a script over files
    # a reader cannot obtain, which is how it went stale unnoticed.
    import json, datetime, os
    out = os.path.join(os.path.dirname(__file__), "..", "..", "results", "runs",
                       "redaction_counts.json")
    with open(os.path.abspath(out), "w") as f:
        json.dump({"generated": datetime.datetime.now(datetime.UTC).isoformat(timespec="seconds"),
                   "source": "redaction_logs/, one trajectory per episode",
                   "note": "Table 4.4. Counts only; trajectories withheld per section 3.11.",
