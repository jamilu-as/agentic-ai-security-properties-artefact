...
out = [
    ...
    "Raw model outputs are gitignored — proposal Section 4.6: *\"raw model outputs withheld to",
    ...
    "",
    f"{len(ROWS)} run rows over {len({r['cell'] for r in ROWS})} distinct cells, from "
    f"{len(RUNS)} run files. Regenerated {datetime.datetime.now(datetime.UTC):%Y-%m-%dT%H:%M:%SZ}.",
    "",
...
print(f"MANIFEST.md: {len(ROWS)} run rows over {len({r[chr(39)+chr(39)] if False else r['cell'] for r in ROWS})} cells, from {len(RUNS)} run files")
