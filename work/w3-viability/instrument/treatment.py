...
class ControlMeasurement:
    ...
    residual_static: Optional[float] = None   # a_c under the static regime (R1); lift needs it
    ...
    @property
    def adaptive_lift_pp(self) -> Optional[float]:
        """Scientific force as pre-registered: R2 minus R1 residual success, in points.

        An earlier version returned residual/undefended, which is pass-through, not
        lift: a control passing half of what reaches it graded weak whatever the adaptive
        regime did. Without a static residual the force is unmeasured, and None is
        returned so the caller declines rather than grades the wrong quantity.
        """
        if self.residual_static is None:
            return None
        return (self.residual - self.residual_static) * 100.0
...
def decide_for_surface(surface: Dict,
    ...
    for prop in surface["properties"]:
        ...
        for stack in candidates:
            ...
            lift = _lift_pp(m)
            if lift is None:
                d = _Undetermined("scientific force unmeasured: no static residual to take lift from")
            else:
                prof = Profile(adaptive_lift=lift, utility_cost=m.utility_cost_pp,
...
@dataclass
class _Undetermined:
    """Stands in for a Decision when a force cannot be graded from what was measured."""
    rationale: str
    treatment: str = "undetermined"


def _lift_pp(m: ControlMeasurement) -> Optional[float]:
    """Scientific force in the cut-points' units: adaptive lift, R2 minus R1, in points."""
    return m.adaptive_lift_pp
