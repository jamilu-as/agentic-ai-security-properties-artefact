...
from derivation import derive_from_architecture, atlas_baseline, atlas_component_baseline  # noqa: E402
...
def main():
    ...
    # -- a fairer comparator: the catalogue behind a component-presence filter ------
    comp_base = {s: atlas_component_baseline(surfaces[s]["architecture"], lib) for s in SUITES}
    comp_j = [jaccard(comp_base[a], comp_base[b]) for a, b in pairs]
    comp_distinct = len({tuple(v) for v in comp_base.values()})
    print(f"\nCOMPONENT-FILTERED BASELINE (catalogue + presence filter, no reachability):")
    for s in SUITES:
        print(f"  {s:<11}{len(comp_base[s]):>4} techniques")
    print(f"  mean Jaccard {mean(comp_j):.3f}; distinct {comp_distinct} of {len(SUITES)}")
    ...
    # the component filter sees components, not ingress: the air-gapped variant keeps them all
    air = derive_from_architecture(deployments.vary(probe, ptools, env_change=deployments.VARIATIONS["air-gapped, staff only"]), lib)
    comp_air = atlas_component_baseline(air["architecture"], lib)
    print(f"  component-filtered baseline, air-gapped variant -> {len(comp_air)} techniques (derivation -> {len(air['properties'])} properties)")
    ...
    json.dump({"atlas": audit, "surfaces": surfaces, "coverage": cov, "sensitivity": sens,
               "discrimination": {"mean_jaccard_properties": mean(derived_j),
                                  ...
                                  "component_baseline": {"per_suite": {s: len(v) for s, v in comp_base.items()},
                                                         "mean_jaccard": mean(comp_j),
                                                         "distinct_surfaces": comp_distinct,
                                                         "air_gapped_variant": len(comp_air)}}},
              open(out, "w"), indent=1)
