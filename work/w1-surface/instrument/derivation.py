...
# --- The vocabulary. Chapter 2 Section 2.2. Additions require a declared revision. ---
...
# trust laundering is memory + peer agents (COMPOSITIONS above, and the analyst
# materials). An earlier mapping keyed it on {retains, plans}, which every suite with a
# planner and a readable file satisfied; the discrimination results before 2 Sep 2026
# raised it for all six deployments on that basis.
...
def derive_from_architecture(arch, lib=None) -> Dict[str, Any]:
    ...
    if "retains" in reach_caps and "peer_agents" in clusters:
        spec = atlas_map.COMPOSITIONAL_TO_ATLAS.get("trust laundering", {"partial": [], "why": ""})
        comp.append({"property": "trust laundering", "from": ["peer_agents", "retains"],
                     "atlas_partial": [lib.describe(t) for t in spec["partial"]],
                     "atlas_covers_composition": False, "gap": spec["why"]})
...
# Which component kind a technique presupposes, by ATLAS technique name. This is the
# cheapest applicability procedure a practitioner could put in front of the catalogue:
# drop what the deployment has no component for. It conditions on presence, not on
# reachability, which is the difference the derivation's reachability rule adds.
_COMPONENT_KEYWORDS = (
    ("rag",          "retrieves"),
    ("memory",       "retains"),
    ("chat history", "retains"),
    ("context poisoning", "retains"),
    ("tool",         "acts"),
    ("supply chain", "installs"),
    ("rug pull",     "installs"),
    ("poisoned",     "installs"),
    ("publish",      "installs"),
    ("deploy ai agent", "peers"),
    ("call chains",  "peers"),
    ("local ai agent", "peers"),
)


def atlas_component_baseline(arch_summary: Dict, lib=None) -> List[str]:
    """The enumerative comparator with a component filter: every agentic technique
    ATLAS carries whose presupposed component the deployment declares. Takes the
    `architecture` summary the derivation returns, so it conditions on the same
    inputs, minus reachability."""
    caps = set(arch_summary.get("capabilities", []))
    if arch_summary.get("topology", "single-agent") != "single-agent":
        caps.add("peers")
    lib = lib or __import__("atlas_map").AtlasLibrary()
    out = []
    for tid in atlas_baseline(lib):
        name = lib.techniques[tid]["name"].lower()
        needs = {cap for kw, cap in _COMPONENT_KEYWORDS if kw in name}
        if not needs or needs <= caps:
            out.append(tid)
    return sorted(out)
