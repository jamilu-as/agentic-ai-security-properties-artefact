...
# ---------------------------------------------------------------------------
# Monotonicity counter-cases. The ceiling rho* <= 1/max(r1,r2) holds only where
# a composed arm is no more permeable than its most permeable component. These
# are the single defences that pass MORE than undefended, so the assumption is
# an assumption. Section 4.II.c cites these figures; printing them here means the
# citation resolves to script output rather than to a derivation a reader must
# redo.
print("\n" + "=" * 76)
print("MONOTONICITY COUNTER-CASES  —  single defences with r > 1")
print("=" * 76)
for unit in ("task", "attempt"):
    print(f"\n  unit = {unit}")
    for suite in sorted({r["suite"] for r in ROWS}):
        ...
        if not a0:
            continue
        for d in sorted({r["defense"] for r in ROWS if r["suite"] == suite}):
            if d == "no_defense":
                continue
            a = rate(suite, d, unit)
            if a is None:
            ...
            r_ = a / a0
            if r_ > 1.0:
                print(f"    {suite:8} {d:14} a={a:.4f}  a0={a0:.4f}  r = {r_:.3f}")
