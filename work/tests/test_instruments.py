...
print("\n=== treatment (RQ3) ===")
a0 = 0.70
M = {"spotlighting": CM(["spotlighting"], .52, a0, 2., .6, residual_static=.30),
     "piguard": CM(["piguard"], .38, a0, 4., 1.2, residual_static=.25),
     "camel": CM(["camel"], .21, a0, 11., 2.4, residual_static=.05),
     "camel+piguard": CM(["camel", "piguard"], .16, a0, 15., 3.1, rho_star=1.40, residual_static=.04)}
check("a stack with no static residual is declined, not graded on pass-through",
      any(x.treatment == "undetermined" and x.recommended for x in decide_for_surface(
          derive_from_architecture(dep.build("shopping", ["send_money", "browse_webpage", "search_items"]), lib),
